...
export function BlockCard({ block, accent, onToggle, onDelete, onEdit }: BlockCardProps) {
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow:  block.isFeatured && !isDragging ? `0 0 12px ${accent}26` : 'none',
        borderColor: block.isFeatured ? `${accent}73` : 'rgba(255,255,255,0.06)'
      }}
    ...
    >
      ...
      <div
        ...
        style={{ background: isActive ? iconAccent : '#333' }}
      />
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <button
          ...
          disabled={busy}
          className="w-7 h-7 rounded-full flex items-center justify-center text-slate-200"
          style={{
            background: isActive ? `${iconAccent}18` : 'rgba(255,255,255,0.06)',
          }}
          aria-label={isActive ? 'Desactivar' : 'Activar'}
          title={isActive ? 'Desactivar' : 'Activar'}
        >
          {isActive
            ? <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0zM2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
